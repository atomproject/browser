...
/**
 * Do things that only need to be once when creating a new project
 * Install dependencies, copy things and vulcanize.
 * @param {Array}     log logger
 * @return {undefined}
 */
...
let generateSite = Q.async(function*(log, argvConfig) {
  ...
  yield installAllElements(log, config.elements);

  // get element ids if available to used for showing build status
  yield getElementIds(log, config.showBuildStatus, config.elements);

  log('Property: Generate the property.json if absent');
  yield generateProperties(log, config.elementsWithDemo);

  log('Demo: Generate demo file if absent');
  yield createDemos(log, config.elementsWithDemo);

  yield syncGeneratedFiles(log, config.elementsWithDemo);
