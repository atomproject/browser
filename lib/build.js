...
let createElementPage = Q.async(function* (log, elContext, config) {
...
let createPage = Q.async(function* (log, page, config) {
  ...
  let context = {site: config, page: page};
  ...
  let pageContent = yield renderLayout(queue, context);
  ...
  log(`Build: ${pagePath}`);
  yield fs.write(pagePath, pageContent);
});

let build = Q.async(function* (log, config, pages) {
  // `config` is extended with `pages` and `pageMenu` properties
  // since they are extended here they can only be used in this module
  pages = pages
    .map(p => {
  ...
  // TODO: move to `config.js`, you should be able to deduce everything
  // related to config from that single file
  ...
  let elPagesP = config.elements.map(elContext => {
    return createElementPage(log, elContext, config);
  });

  //create other pages
  let pagesP = pages.map(page => createPage(log, page, config));
